...
<head>
    ...
    <title>GreenBoard - Welcome Back</title>
...
<nav class="absolute top-0 right-0 z-50 p-6 flex items-center gap-4">
    ...
    <button onclick="toggleLanguage()" class="w-10 h-10 flex items-center justify-center rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white lg:text-slate-700 lg:dark:text-slate-200 hover:border-primary">
        <span class="text-xs font-black" id="lang-label">EN</span>
    </button>
...
<div class="flex min-h-screen w-full flex-col lg:flex-row">
    <div id="hero-section" class="flex lg:w-1/2 relative overflow-hidden bg-primary/10 h-screen lg:min-h-screen cursor-pointer lg:cursor-default group" onclick="showForm()">
        ...
        <div class="relative z-20 flex flex-col justify-end p-10 lg:p-16 w-full h-full">
            ...
            <h1 id="hero-title" class="text-white text-4xl lg:text-5xl font-black leading-tight mb-4">Welcome back</h1>
            <p id="hero-desc" class="text-slate-200 text-lg max-w-md mb-6 lg:mb-0">Log in to continue your sustainable journey and connect with the community.</p>
            
            <div id="tap-indicator" class="lg:hidden flex items-center text-primary font-bold animate-bounce mt-4">
    ...
    <div id="form-section" class="hidden lg:flex flex-1 flex-col justify-center items-center px-6 py-12 lg:px-20 bg-background-light dark:bg-background-dark fade-in">
        <div class="w-full max-w-[440px]">
            <div class="mb-10 text-center lg:text-left">
                <h2 id="form-title" class="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-2">Login to Account</h2>
                <p id="form-desc" class="text-slate-600 dark:text-slate-400">Enter your credentials to access your dashboard.</p>
            </div>

            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                @csrf
                <div>
                    <label id="label-email" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Email Address</label>
                    <div class="relative">
                ...
                <div>
                    <div class="flex justify-between mb-2">
                        <label id="label-pass" class="block text-sm font-semibold text-slate-700 dark:text-slate-300">Password</label>
                ...
                <button id="btn-login" class="w-full bg-primary text-slate-900 font-extrabold text-base py-4 rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-[0.95]" type="submit">
                    Login Now
                </button>
            </form>
            
            <p class="text-center mt-8 text-slate-600 dark:text-slate-400">
                <span id="text-no-account">Don't have an account?</span>
...
<script>
    ...
    const texts = {
        EN: {
            title: "Welcome back", desc: "Log in to continue your sustainable journey...",
            fTitle: "Login to Account", fDesc: "Enter your credentials...",
            email: "Email Address", pass: "Password", btn: "Login Now", noAcc: "Don't have an account?", tap: "Tap to login"
        },
        ES: {
            title: "Bienvenido de nuevo", desc: "Inicia sesión para continuar tu viaje sostenible...",
            fTitle: "Acceder a tu Cuenta", fDesc: "Ingresa tus credenciales...",
            email: "Correo Electrónico", pass: "Contraseña", btn: "Entrar Ahora", noAcc: "¿No tienes cuenta?", tap: "Toca para entrar"
        }
    };
    ...
    function toggleLanguage() {
        ...
        document.getElementById('hero-title').innerText = l.title;
        document.getElementById('hero-desc').innerText = l.desc;
        document.getElementById('form-title').innerText = l.fTitle;
        document.getElementById('form-desc').innerText = l.fDesc;
        document.getElementById('label-email').innerText = l.email;
        document.getElementById('label-pass').innerText = l.pass;
        document.getElementById('btn-login').innerText = l.btn;
        document.getElementById('text-no-account').innerText = l.noAcc;
        document.getElementById('tap-indicator').innerHTML = `<span class="material-symbols-outlined mr-2">touch_app</span> ${l.tap}`;
    }
</script>
