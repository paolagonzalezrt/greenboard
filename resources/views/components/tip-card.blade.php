...
@php
    $categoryMapping = [
        ...
        'residuos' => ['color' => 'brown', 'label' => 'residuos'],
    ...
    $categoryColors = [
        ...
        'brown' => ['bg' => 'bg-brown-100/90', 'text' => 'text-brown-800'],
...
<div class="bg-white dark:bg-custom-dark-bg rounded-xl sm:rounded-2xl overflow-hidden border border-slate-200 dark:border-custom-gray-border shadow-md hover:shadow-xl transition-all group flex flex-col cursor-pointer h-full" onclick="window.location.href='{{ $id ? route('tips.show', $id) : '#' }}'">
    ...
    <div class="p-4 sm:p-5 lg:p-6 flex-1 flex flex-col min-h-0">
        ...
        <div class="{{ $image ? 'mb-4 sm:mb-6' : 'mb-4' }} flex-1">
            <h4 class="text-base sm:text-lg font-bold leading-tight mb-1.5 sm:mb-2 group-hover:text-primary transition-colors {{ $image ? 'line-clamp-2' : '' }}">{{ $title }}</h4>
            <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-normal leading-relaxed {{ $image ? 'line-clamp-2 sm:line-clamp-3' : '' }}">{{ $description }}</p>
        </div>

        <div class="flex items-center justify-between text-slate-400 mt-auto select-none">
            <div class="flex items-center gap-3 sm:gap-4">
                <div class="flex items-center gap-1 text-slate-400 group relative likers-trigger" data-tip-id="{{ $id }}">
                    <button type="button" class="flex items-center hover:text-primary cursor-pointer transition-colors" onclick="event.stopPropagation(); toggleLike(this)" data-tip-id="{{ $id }}">
                    ...
                    <span class="text-[11px] sm:text-xs font-bold hover:text-primary cursor-pointer transition-colors group-hover:opacity-70" data-like-count onclick="event.stopPropagation(); openLikersModal({{ $id }})">{{ $likes }}</span>
                </div>
